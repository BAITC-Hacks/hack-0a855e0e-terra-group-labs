...
## 2026-09-23 final-hour audit / plan
- Re-read all 8 pages of organizer PDF: the five gates are parquet→three CSV, all 2,248 roles/evidence, documented rules, clusters/hypotheses, and ranked list + directional graph. README, reproducibility and practical value are scored; hosted deployment is not required. Three CSV exports are mandatory, so do not delete `outputs/`; production must regenerate them from tracked raw parquet.
- UI user: bank AML analyst; job: start from the 81 previously identified clients, inspect directed transfers and explain priority. Demo moment: choose a top GID, follow a red direct link to a previously identified client, then inspect a cluster/flow. Design: retain the existing dark investigation console and amber selection, add coral only for direct seed-related evidence; keep the five depth lanes and avoid new screens/dependencies.
- This iteration's priority: fix deterministic cluster-hypothesis false text; make the seed concept plain-language; make cluster leaderboard open cluster supernode and preserve a way back from flow drilldown; support raw-data production start on Windows/Linux and optional Docker; verify README and full QA. No speculative bonus algorithms during final hour.
- Git attribution: organizer commit stays untouched. User identifies first working prompt as 2026-09-23 13:46 local. At the user's explicit later request and organizer permission, the two old-identity commits were rewritten in place to `Zeylan Emir <emir.zeilan@gmail.com>` while preserving their original timestamps and trees; `origin/main` was updated with an exact-SHA force-with-lease. New IDs: `93d87e4` (13:46:42) and `3f24759` (14:04:53). The original head remains locally at `codex/pre-author-rewrite-20260923`; the work-in-progress stash was reapplied after the rewrite. No README attribution section is needed.

## 2026-09-23 final verification
- Checkpoint `81f46a2` was pushed after full `npm run qa` passed (7 backend, 3 frontend unit, 11 Playwright E2E). The checkpoint contains Docker/raw-parquet production start, corrected cluster hypotheses, seed relationship highlighting, browser-history restoration for node and cluster flows, and README requirement mapping.
- Subsequent UI copy polish removes remaining visible `seed` jargon from analytics, filters, footer and depth label; cluster leaderboards/table now explicitly mark prior-identified clients or direct links. Final `npm run qa` passed again: pipeline 0.99s, validator, 7 backend tests, Ruff, 3 Vitest tests, ESLint, frontend build, 11 E2E tests. Only upstream deprecation and Vite bundle-size advisories remain.
- Playwright MCP inspected the production page and cluster analytics at 1440×900; root width 1425px versus 1440px viewport, no horizontal overflow. Docker final image built on Linux, excludes `/app/outputs`, regenerates all CSV from raw parquet in 1.29s at startup, serves `/` HTTP 200 and `/api/health` status `ok`. Container stopped after check.
- Requirements audit: 2,248 role/evidence rows, 91 clusters, 50 ranked nodes; all 3 mandatory CSV remain in repository and are regenerated in production. Hosted public deployment and 1M-node benchmark are not claimed. Do not publish organizer parquet data in a public Docker registry without authorization.

## 2026-09-23 final ten-minute update
- README scaling section now describes the incremental MVP route: preserve the analyst workflow/API contract, fetch graph slices through REST with future pagination, and profile/optimize in-memory batch metrics before claiming 1M-node performance. Optional work is stated honestly.
- Added a deterministic next-data-request hint to the existing seed/depth-4 coverage warning, supporting the optional completeness criterion without changing role/score calculations. Frontend lint/build and two targeted Playwright E2E cases passed.
- Docker Desktop final image rebuilt after that UI change; image has no `/app/outputs`, startup regenerated CSV from raw parquet in 1.22s, `/` returned HTTP 200, `/api/health` returned `ok`, container stopped.
